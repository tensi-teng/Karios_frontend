    ...
    <div className="pt-28 pb-20 px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-8 items-center justify-between">
        <div className="flex items-center gap-3 w-full lg:max-w-md">
          ...
          <button className="p-3.5 glass border border-white/10 rounded-2xl text-gray-400 hover:text-white hover:border-indigo-500/50 transition-all">
            <Filter className="w-5 h-5" />
          </button>
        </div>

        <div className="flex items-center gap-2 p-1.5 glass rounded-full border border-white/10 overflow-x-auto scrollbar-hide">
          <FilterTab active={filterType === 'ALL'} label="Total Events" onClick={() => setFilterType('ALL')} />
          <FilterTab active={filterType === 'SEAL'} label="Protocol Seals" onClick={() => setFilterType('SEAL')} />
          <FilterTab active={filterType === 'BENEFICIARY'} label="Total Beneficiaries" onClick={() => setFilterType('BENEFICIARY')} />
          <FilterTab active={filterType === 'CLAIM'} label="Claim Attempt" onClick={() => setFilterType('CLAIM')} />
          <FilterTab active={filterType === 'FILE'} label="Files" onClick={() => setFilterType('FILE')} />
        </div>
      </div>
...
const FilterTab: React.FC<{ active: boolean, label: string, onClick: () => void }> = ({ active, label, onClick }) => (
  <button
    onClick={onClick}
    className={`px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${active ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-white/5'
      }`}
  >
    {label}
  </button>
