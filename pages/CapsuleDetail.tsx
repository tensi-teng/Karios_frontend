...
import { ArrowLeft, Clock, Shield, Bell, Heart, User, ChevronRight, Lock, Unlock, CheckCircle, Database, Activity, AlertTriangle, Users, Settings, History, Info, Edit3, Save, Zap, ExternalLink, ShieldCheck } from 'lucide-react';
         ...
         <div className="grid lg:grid-cols-12 gap-10">
               ...
               <div className="glass p-12 rounded-[3.5rem] border border-white/10 relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                     <div className="flex flex-wrap items-center gap-4 mb-10">
                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border shadow-sm ${capsule.state === CapsuleState.ACTIVE ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20'
                           }`}>
                           {capsule.state}
                        </span>
                     ...
                     <h1 className="text-4xl md:text-5xl font-bold font-display mb-6 text-white leading-tight tracking-tight">{capsule.title}</h1>
                     <p className="text-gray-400 leading-relaxed mb-12 text-xl max-w-2xl font-medium">{capsule.description}</p>

                     <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12 bg-white/[0.02] p-8 rounded-[2.5rem] border border-white/5">
                        <div className="space-y-1">
                           <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Unlock Date</p>
                           <p className="text-sm font-bold text-white">{new Date(capsule.unlockTime || Date.now() + 31536000000).toLocaleDateString()}</p>
                           <p className="text-[9px] text-indigo-400 font-bold">{timeLeft}</p>
                        </div>
                        <div className="space-y-1">
                           <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Beneficiaries</p>
                           <p className="text-sm font-bold text-white">{capsule.beneficiaries.length} Designated</p>
                        </div>
                        <div className="space-y-1">
                           <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Files Included</p>
                           <p className="text-sm font-bold text-white">4 Objects (2.4 GB)</p>
                        </div>
                        <div className="space-y-1">
                           <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Rules Active</p>
                           <p className="text-sm font-bold text-white">{capsule.unlockRules.length} Logic Blocks</p>
                        </div>
                     </div>
                        ...
                        {activeTab === 'integrity' && (
                           <motion.div key="integrity" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                              <div className="bg-black/40 rounded-[3.5rem] p-12 border border-white/5 space-y-12 relative overflow-hidden shadow-2xl">
                                 ...
                                 <div className="flex items-center gap-4 p-6 bg-green-500/5 border border-green-500/20 rounded-3xl mb-10">
                                    <div className="p-3 bg-green-500/10 rounded-2xl">
                                       <ShieldCheck className="text-green-400 w-8 h-8" />
                                    </div>
                                    <div>
                                       <p className="text-[10px] font-black text-green-500 uppercase tracking-widest">Integrity Status</p>
                                       <h4 className="text-2xl font-bold text-white font-display">TAMPER-PROOF SEALS ACTIVE</h4>
                                    </div>
                                 </div>
                                 <IntegrityRow label="Walrus Distributed ID" value={capsule.blobId || 'ANCHORED_PROTOCOL_ID'} />
                                 <IntegrityRow label="Merkle Root Hash" value="0x77ae8899cc66aa00bb11cc0099ff2288" />
                                 <IntegrityRow label="Network Consensus" value="99.98% Healthy Nodes" />
                                 ...
                                 <div className="pt-10 border-t border-white/5 flex items-center gap-5">
                                    ...
                                    <span className="text-xs font-black text-gray-400 uppercase tracking-[0.3em]">Verified Cryptographic Proof Established on Sui Mainnet</span>
                                 </div>
                              </div>
                           </motion.div>
                        )}
            ...
            <div className="lg:col-span-4 space-y-10">
               ...
               <div className="glass p-10 rounded-[3.5rem] border border-indigo-500/30 bg-indigo-500/5 text-center relative overflow-hidden group shadow-2xl">
                  ...
                  <div className="text-5xl font-bold font-display text-white mb-6 tracking-tighter tabular-nums">{timeLeft}</div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-8">Until next safety ping</div>

                  <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden mb-10 shadow-inner">
                     <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: timeLeft.includes('Grace') ? '100%' : '85%' }}
                        className={`h-full ${timeLeft.includes('Grace') ? 'bg-red-500' : 'bg-indigo-500'} shadow-[0_0_20px_rgba(99,102,241,0.6)]`}
                     />
                  </div>

                  <div className="min-h-[60px] flex items-center justify-center">
                     {timeLeft.includes('0d 0h 0m') || timeLeft.includes('Grace') ? (
                        <button
                           onClick={onPing}
                           className="text-[10px] bg-indigo-600 text-white font-black uppercase tracking-widest px-10 py-5 rounded-[1.8rem] hover:bg-indigo-500 transition-all w-full flex items-center justify-center gap-3 glow shadow-xl animate-bounce"
                        >
                           <Zap className="w-4 h-4 fill-current" />
                           Pulse Required: PING NOW
                        </button>
                     ) : (
                        <div className="text-[10px] text-gray-600 font-bold uppercase tracking-widest bg-white/5 px-6 py-4 rounded-2xl border border-white/5">
                           Pulse synchronised
                        </div>
                     )}
                  </div>
               ...
               <div className="glass p-10 rounded-[3.5rem] border border-white/10 bg-white/[0.01]">
                  <div className="flex items-center gap-3 mb-10">
                     ...
                     <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Recipient Status</span>
                  </div>
