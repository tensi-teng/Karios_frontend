    ...
    <div className="pt-28 pb-20 px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-2 p-1.5 glass rounded-full border border-white/10 overflow-x-auto scrollbar-hide max-w-full">
          ...
          <FilterButton active={filter === 'PERSONAL'} label="Personal" onClick={() => setFilter('PERSONAL')} />
...
const FilterButton: React.FC<{ active: boolean; label: string; onClick: () => void }> = ({ active, label, onClick }) => (
  <button
    onClick={onClick}
    className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${active
...
const CapsuleCard: React.FC<{ capsule: Capsule; onClick: () => void }> = ({ capsule, onClick }) => {
  ...
  return (
    <motion.div
      ...
      className="group p-8 rounded-[3.5rem] glass border border-white/10 hover:border-indigo-500/50 transition-all cursor-pointer relative overflow-hidden flex flex-col min-h-[360px]"
      onClick={onClick}
    >
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        ...
        <div className="flex flex-col items-end">
          <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest mb-2 border ${capsule.isActivated
            ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'
            : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20'
            }`}>
            {capsule.isActivated ? 'Sealed Protocol' : 'Mutable Draft'}
          </span>
          <div className="flex items-center gap-1 mt-1 text-[8px] font-black text-indigo-400 uppercase tracking-widest">
            <Activity className="w-3 h-3" />
            Stability Index: {(capsule.healthScore * 0.98).toFixed(1)}%
          </div>
        </div>
      </div>

      <h4 className="text-2xl font-bold mb-2 group-hover:text-indigo-400 transition-colors font-display text-white leading-tight">{capsule.title}</h4>
      <p className="text-sm text-gray-500 line-clamp-2 mb-6 leading-relaxed flex-grow font-medium">{capsule.description}</p>
      ...
      <div className="space-y-4 pt-6 border-t border-white/5 relative z-10">
        <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
          ...
          <span className={isUrgent ? 'text-yellow-500 font-bold' : 'text-indigo-400'}>
            <CountdownTimer targetDate={nextPing} />
          </span>
        </div>
